...
//Code Here
// let evenNumbers = mixedNumbers.filter(function(elem, ind) {
//   return elem % 2 === 0;
// });

let evenNumbers = mixedNumbers.filter((elem, ind) => elem % 2 === 0);
...
//Code Here
let postTaxPrices = prices.map((elem, ind) => elem * 1.07);
...
//Code Here
let totalPopulation = populations.reduce(function(total, elem, ind) {
  return total + elem;
}, 0);
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(elem, ind) {
  return elem.CP > 200;
});
...
let ordersTotal = orders.map(orders => {
  orders.price * orders.tax;
});
ordersTotal.reduce((total, element) => total + element);
